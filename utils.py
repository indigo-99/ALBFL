...
FORMULAS = [
    ...
    'er5a', 'er5b', 'er5c', 'gp2', 'gp3', 'gp13', 'gp19'
]
SPECTRUM = ['ep', 'ef', 'np', 'nf']
CODE = ['code']
METRICS1 = ['level', 'nLines', 'nComments', 'nTokens', 'nChars']
METRICS2 = [
    'nKeyword', 'nModifier', 'nBasicType', 'nSeparator', 'nOperator',
    'nAnnotation', 'nIdentifier', 'nInteger', 'nPoint', 'nBoolean',
    'nCharacter', 'nString', 'nNull'
]
METRICS3 = [
    'isAnnotation', 'isNone', 'isAssignment', 'isMethodInvocation',
    'isSuperConstructorInvocation', 'isMemberReference', 'isBinaryOperation'
    'isExplicitConstructorInvocation', 'isThis', 'isSuperMethodInvocation',
    'isClassCreator'
    'isCast'
    'isClassReference', 'isTernaryExpression', 'isLiteral', 'isStatement',
    'isReturnStatement', 'isBreakStatement', 'isStatementExpression',
    'isContinueStatement', 'isThrowStatement', 'isIfStatement',
    'isAssertStatement', 'isForStatement', 'isBlockStatement'
]
...
def get_src_dir(project, id):
    ...
    if (project == 'Math' and int(id) >= 85) or (project == 'Lang'
                                                 and int(id) >= 36):
        return 'src/java'
...
def parse_test_summary(line, n_elements):
    TestSummary = collections.namedtuple('TestSummary',
                                         ('triggering', 'covered_elements'))
    words = line.strip().split(' ')
    coverages, sign = words[:-1], words[-1]
    if len(coverages) != n_elements:
        raise ValueError(
            'Expected {expected} elements in each row, got {actual} in {line!r}'
            .format(expected=n_elements, actual=len(coverages), line=line))
    return TestSummary(triggering=(sign == '-'),
                       covered_elements=set(i for i in range(len(words))
                                            if words[i] == '1'))


def tally_matrix(matrix_file, n_elements):
    PassFailTally = collections.namedtuple(
        'PassFailTally',
        ('n_elements', 'passed', 'failed', 'totalpassed', 'totalfailed'))
    summaries = (parse_test_summary(line.decode('utf-8'), n_elements)
...
def calc_metric_svm(n):
    ...
    for i in range(n):
        data = {}
        curdir = 'data/cross_dataabfl/{}'.format(i)
        ...
        for key in data.keys():
            ...
            if count != 0:
                ...
                while end + 1 < len(sorted_lst):
                    cur_Einspect, count, cur_end = get_Einspect(
                        sorted_lst[end + 1:])
                    if cur_Einspect == -1:
                        break
                    Einspects.extend(end + 1 + cur_Einspect
                ...
                for i in range(1, int(qid2lines[qid]) + 1):
                    ...
                    Pi = len([inspect
                              for inspect in inspects if inspect <= i]) / i
    ...
    top = []
    top.append(
        len(list(filter(lambda item: item <= 1 and item > 0, Epos_list))))
    top.append(
        len(list(filter(lambda item: item <= 3 and item > 0, Epos_list))))
    top.append(
        len(list(filter(lambda item: item <= 5 and item > 0, Epos_list))))
    top.append(
        len(list(filter(lambda item: item <= 10 and item > 0, Epos_list))))
...
def calc_metric_method(n, predfilename='rank-pred.dat'):
    qid2methods = {}
    for line in open('data/qid-methods.csv'):
        items = line.strip().split(',')
        qid2methods[items[0]] = items[1]

    qid2nfault = {}
    for line in open('data/qid-nfault-method.csv'):
        items = line.strip().split(',')
        qid2nfault[items[0]] = items[1]
    ...
    for i in range(n):
        data = {}
        curdir = 'data/cross_datasvm/{}'.format(i)
        #curdir = 'data/cross_dataabfl/{}'.format(i)
        testcsv = pd.read_csv('{}/test.csv'.format(curdir))
        #test_data = open('{}/test.dat'.format(curdir)).readlines()
        pred_data = open('{}/'.format(curdir) + predfilename).readlines()
        methodinfo = list(testcsv['line'].values)
        isfault = list(testcsv['faulty'].values)
        qidlist = list(testcsv['qid'].values)
        methodname = [m.split('.java#')[0] for m in methodinfo]
        qid2methodname = {}
        for i in range(len(pred_data)):
            score = float(pred_data[i])
            is_fault = int(isfault[i])
            qid = str(qidlist[i])
            method = methodname[i]
            if qid not in qid2methodname:
                qid2methodname[qid] = {}
            if method not in qid2methodname[qid]:
                qid2methodname[qid][method] = [score, is_fault]
            elif score > qid2methodname[qid][method][0]:
                qid2methodname[qid][method][0] = score
            elif is_fault == 1 and qid2methodname[qid][method][1] == 0:
                qid2methodname[qid][method][1] = 1
        for qid in qid2methodname:
            if qid not in data:
                data[qid] = []
            for method in qid2methodname[qid]:
                item = {
                    'score': qid2methodname[qid][method][0],
                    'is_fault': qid2methodname[qid][method][1]
                }
                data[qid].append(item)
        ...
        for key in data.keys():
            ...
            # calc EXAM
            qid = key
            EXAM = Einspect / float(qid2methods[qid])
            EXAM_list.append(EXAM)
            '''# calc AP
            if count != 0:
                Einspects = [Einspect for _ in range(count)]
...
def calc_metric(n, predfilename='ranknet-pred.dat'):
    qid2lines = {}
    for line in open('data/qid-lines.csv'):
        items = line.strip().split(',')
        qid2lines[items[0]] = items[1]
    ...
    for i in range(n):
        data = {}
        curdir = 'data/cross_datasvm/{}'.format(i)
        test_data = pd.read_csv('{}/test.csv'.format(curdir))
        test_label = test_data['faulty']
        qidlist = test_data['qid']
        #test_data = open('{}/test.dat'.format(curdir)).readlines()
        pred_data = open('{}/'.format(curdir) + predfilename).readlines()
        for i in range(len(pred_data)):
            #test_line = test_data[i]
            is_fault = test_label[i]
            pred_line = pred_data[i]
            #is_fault = int(test_line.split(' ', 1)[0])  # label
            #qid = test_line.split(' ', 2)[1]
            qid = 'qid:' + str(qidlist[i])
            score = float(pred_line)  # pred
            if qid not in data:
                data[qid] = []
            item = {'score': score, 'is_fault': is_fault}
            data[qid].append(item)
...
def prepare_data(vocab_size):
    ...
    pairs = extract_pairs(merge_norm['faulty'].values,
                          merge_norm['qid'].values)

    tokenizer = Tokenizer(num_words=vocab_size)  #vectorize a sentence
    tokenizer.fit_on_texts(merge_norm['code'].to_list())  #get vocab from codes
    pickle.dump(tokenizer,
                open('data/tokenizer.pickle', 'wb'),
                protocol=pickle.HIGHEST_PROTOCOL)  #store vector

    merge_norm.drop(['project', 'line', 'faulty'], axis=1, inplace=True)
    colnames = ['label'] + [col + '_1' for col in merge_norm.columns
                            ] + [col + '_2' for col in merge_norm.columns]
    ofile = csv.writer(open('data/pairs.csv', 'w', encoding='utf-8'))
    ofile.writerow(colnames)

    data = merge_norm.values.tolist()
    for pair in tqdm(pairs):
        label = random.choice(
            [0, 1])  #random generate positive and negtive sample pairs
...
def split_data(n):
    ...
    #for line in open('data/test-qids.csv'): # from file 2-svm_autoencoder.py result (svm method)
    for line in open('data/test-qidssvm.csv'):
        items = line.strip().split(',')
    ...
    for i in range(n):
        ...
        train_csv = csv.DictWriter(open('{}/train.csv'.format(data_dir),
                                        'w',
                                        encoding='utf-8'),
                                   fieldnames=data.fieldnames)
        test_csv = csv.DictWriter(open('{}/test.csv'.format(data_dir),
                                       'w',
                                       encoding='utf-8'),
                                  fieldnames=data.fieldnames)
        ...
        train_csv = csv.DictWriter(open('{}/train_pairs.csv'.format(data_dir),
                                        'w',
                                        encoding='utf-8'),
                                   fieldnames=data_pairs.fieldnames)
        test_csv = csv.DictWriter(open('{}/test_pairs.csv'.format(data_dir),
                                       'w',
                                       encoding='utf-8'),
                                  fieldnames=data_pairs.fieldnames)
        train_csv.writeheader()
